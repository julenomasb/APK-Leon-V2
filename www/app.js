...
const DEFAULT_MAINTENANCE = [
  ...
  { id: "itv", name: "ITV", intervalKm: null, intervalMonths: 12, lastKm: null, lastDate: null, custom: false, nextOverride: null },
  { id: "seguro", name: "Seguro del coche", intervalKm: null, intervalMonths: 12, lastKm: null, lastDate: null, custom: false, nextOverride: null },
];
...
function loadState() {
  try {
    ...
    let maintenance = parsed.maintenance && parsed.maintenance.length ? parsed.maintenance : base.maintenance;
    // añade mantenimientos nuevos de fábrica que no existieran todavía en datos guardados de una versión anterior
    const knownIds = new Set(maintenance.map((m) => m.id));
    DEFAULT_MAINTENANCE.forEach((d) => {
      if (!knownIds.has(d.id)) maintenance.push(JSON.parse(JSON.stringify(d)));
    });
...
function formatMoneyPrecise(v) {
  return Number(v || 0).toLocaleString("es-ES", { style: "currency", currency: "EUR", minimumFractionDigits: 3, maximumFractionDigits: 3 });
}

function computeCostPerKm() {
  let minKm = null;
  state.expenses.forEach((e) => {
    if (e.km != null) minKm = minKm === null ? e.km : Math.min(minKm, e.km);
  });
  state.fuel.forEach((f) => {
    minKm = minKm === null ? f.km : Math.min(minKm, f.km);
  });
  if (minKm === null || state.km <= minKm) return null;
  const totalSpend = state.expenses.reduce((s, e) => s + e.amount, 0) + state.fuel.reduce((s, f) => s + (f.cost || 0), 0);
  return totalSpend / (state.km - minKm);
}
...
function maintenanceStatus(item) {
  ...
  if (item.lastKm === null && item.lastDate === null && !item.nextOverride) {
  ...
  // Si el usuario conoce la fecha exacta del próximo vencimiento (p. ej. ITV o seguro),
  // esa fecha manda sobre el cálculo automático a partir del último cambio + intervalo.
  if (item.nextOverride) {
    nextDate = item.nextOverride;
  } else if (item.intervalMonths && item.lastDate) {
...
function renderHome() {
  ...
  statGrid.innerHTML = `
    ...
    <div class="stat-card wide">
      <div class="label">Coste medio por km recorrido</div>
      <div class="value">${costPerKm !== null ? formatMoneyPrecise(costPerKm) + "/km" : "Sin datos suficientes"}</div>
    </div>
  `;
...
function openMaintenanceForm(id) {
  ...
  openModal(
    ...
    <div class="field-row">
      <label>Próximo vencimiento conocido (opcional)</label>
      <input type="date" id="f_nextoverride" value="${item.nextOverride || ""}">
    </div>
    <p class="hint">Si conoces la fecha exacta (por ejemplo la de la próxima ITV o el recibo del seguro), indícala aquí y tendrá prioridad sobre el cálculo por intervalo.</p>
  ...
  `,
    (root) => {
      ...
      root.querySelector("#f_save").addEventListener("click", () => {
        ...
        item.nextOverride = root.querySelector("#f_nextoverride").value || null;
        saveState();
        renderAll();
        closeModal();
...
const CAL_TYPES = {
  gasto: { color: "var(--cal-gasto)", label: "Gastos" },
  fuel: { color: "var(--cal-fuel)", label: "Repostajes" },
  maint: { color: "var(--cal-maint)", label: "Mantenimiento" },
  issue: { color: "var(--cal-issue)", label: "Averías" },
  doc: { color: "var(--cal-doc)", label: "ITV / Seguro" },
};
...
function getCalendarEvents() {
  // devuelve un Map de "YYYY-MM-DD" -> [{type, title, detail}]
  const map = {};
  const add = (dateStr, type, title, detail) => {
    if (!dateStr) return;
    if (!map[dateStr]) map[dateStr] = [];
    map[dateStr].push({ type, title, detail });
  };

  state.expenses.forEach((e) => add(e.date, "gasto", e.desc || e.category, formatMoney(e.amount)));
  state.fuel.forEach((f) => add(f.date, "fuel", `${f.liters.toFixed(1)} L repostados`, formatKm(f.km)));
  state.issues.forEach((i) => add(i.createdAt, "issue", i.title, "Avería creada"));
  state.maintenance.forEach((m) => {
    const s = maintenanceStatus(m);
    if (s.nextDate) add(s.nextDate, m.id === "itv" || m.id === "seguro" ? "doc" : "maint", m.name, s.label);
  });
...
function renderCalendar() {
  ...
  document.getElementById("calMonthLabel").textContent = calDate.toLocaleDateString("es-ES", { month: "long", year: "numeric" });
  ...
  for (let i = 0; i < startOffset; i++) {
    cells.push({ day: daysInPrevMonth - startOffset + 1 + i, outside: true });
  }
  for (let d = 1; d <= daysInMonth; d++) {
    cells.push({ day: d, outside: false, dateStr: `${year}-${String(month + 1).padStart(2, "0")}-${String(d).padStart(2, "0")}` });
  }
  ...
  grid.innerHTML = cells
    .map((c) => {
      if (c.outside) return `<div class="cal-day outside"><span>${c.day}</span></div>`;
      const dayEvents = events[c.dateStr] || [];
      const dots = dayEvents
        .slice(0, 5)
        .map((ev) => `<span class="cal-dot" style="background:${CAL_TYPES[ev.type].color}"></span>`)
        .join("");
      const isToday = c.dateStr === todayStr;
      return `<div class="cal-day ${isToday ? "today" : ""} ${dayEvents.length ? "has-events" : ""}" data-date="${c.dateStr}">
        <span>${c.day}</span>
  ...
  grid.querySelectorAll(".cal-day.has-events").forEach((el) => {
    el.addEventListener("click", () => openDayModal(el.dataset.date, events[el.dataset.date]));
  });

  document.getElementById("calLegend").innerHTML = Object.values(CAL_TYPES)
    .map((t) => `<div class="cal-legend-item"><span class="cal-legend-dot" style="background:${t.color}"></span>${t.label}</div>`)
    .join("");
}
...
function openDayModal(dateStr, dayEvents) {
  const list = (dayEvents || [])
    ...
    <div class="cal-day-event">
      <span class="dot" style="background:${CAL_TYPES[ev.type].color}"></span>
      <div class="li-main">
        <div class="li-title">${ev.title}</div>
        <div class="li-sub">${ev.detail || CAL_TYPES[ev.type].label}</div>
      </div>
    </div>`
    )
    .join("");
  openModal(formatDate(dateStr), list || `<div class="empty-state">Sin eventos este día</div>`);
}
...
async function sendChatMessage() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        pushSystemMsg("Tu clave de API no es válida o ha caducado. Revísala en la pestaña 'Más' → Asistente de averías.");
      } else if (res.status === 403) {
        pushSystemMsg("La clave de API no tiene permiso para usar este modelo o no tiene saldo/facturación activa en la consola de Anthropic.");
      } else if (res.status === 429) {
        pushSystemMsg("Se ha alcanzado el límite de peticiones de tu cuenta. Espera un momento y vuelve a intentarlo.");
      } else {
        pushSystemMsg(`No se pudo contactar con la IA (código ${res.status}): ${detail}`);
      }
      return;
    }
    ...
    currentIssue.chat.splice(thinkingIndex, 1);
    pushSystemMsg("Error de conexión al intentar hablar con la IA (" + (e.message || e) + "). Comprueba tu conexión a internet.");
    console.error(e);
  }
}
...
document.getElementById("btnSaveDocs").addEventListener("click", () => {
  ...
  // sincroniza la fecha de ITV y de seguro con sus mantenimientos correspondientes
  const itvItem = state.maintenance.find((m) => m.id === "itv");
  if (itvItem && state.docs.itv) itvItem.nextOverride = state.docs.itv;
  const seguroItem = state.maintenance.find((m) => m.id === "seguro");
  if (seguroItem && state.docs.insuranceDate) seguroItem.nextOverride = state.docs.insuranceDate;
  saveState();
  renderAll();
...
document.getElementById("btnTestKey").addEventListener("click", async () => {
  ...
  if (!key) return toast("Escribe primero tu clave de API");
  ...
  try {
    ...
    if (res.ok) {
      toast("✅ Conexión correcta, la clave funciona");
    } else {
      ...
      alert("Fallo al conectar (código " + res.status + "):\n" + detail);
    }
  } catch (e) {
